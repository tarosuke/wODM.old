	...
	private:
		...
		void UpdateElement(Element& e, float delta) final{
			...
			for(unsigned n(id + 1); n < numOfParticles; ++n){
				...
				e.velocity.x += (ax - (0.00005 * e.position.x)) * delta;
				e.velocity.y += (ay - (0.00005 * e.position.y)) * delta;
				e.velocity.z += (az - (0.00005 * e.position.z)) * delta;
				ee.velocity.x -=ax;
				ee.velocity.y -= ay;
				ee.velocity.z -= az;
			}
